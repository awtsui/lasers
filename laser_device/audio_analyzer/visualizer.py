...
class Spectrum_Visualizer:
    ...
    def __init__(self, ear):
        ...
        )  # Occasionally display Hz tags on the x-axis

        self.fast_bar_colors = [
            list((255 * np.array(self.cm(i))[:3]).astype(int))
            for i in np.linspace(0, 255, self.ear.n_frequency_bins).astype(int)
        ]
        self.slow_bar_colors = [
            list(np.clip((255 * 3.5 * np.array(self.cm(i))[:3]).astype(int), 0, 255))
            for i in np.linspace(0, 255, self.ear.n_frequency_bins).astype(int)
        ]
    ...
    def toggle_history_mode(self):
        ...
        self.bar_width = (
            self.WIDTH / self.ear.n_frequency_bins
        ) - self.inter_bar_distance
    ...
    def start(self):
        ...
        pygame.display.set_caption(
            "Spectrum Analyzer -- (FFT-Peak: %05d Hz)" % self.ear.strongest_frequency
        )
        ...
        for i in range(self.ear.n_frequency_bins):
            ...
            if i % self.tag_every_n_bins == 0:
                ...
                x = (
                    i * (self.WIDTH / self.ear.n_frequency_bins)
        ...
        self.history_button = Button(
            text="Toggle 2D/3D Mode",
            right=self.WIDTH,
            top=0,
            width=round(0.12 * self.WIDTH),
            height=self.button_height,
        )
        self.slow_bar_button = Button(
            text="Toggle Slow Bars",
            right=self.WIDTH,
            top=self.history_button.height,
            width=round(0.12 * self.WIDTH),
            height=self.button_height,
        )
    ...
    def update(self):
        ...
        if np.min(self.ear.bin_mean_values) > 0:
            self.frequency_bin_energies = (
                self.avg_energy_height
                * self.ear.frequency_bin_energies
                / self.ear.bin_mean_values
            )

        if self.plot_audio_history:
            new_w, new_h = int((2 + self.shrink_f) / 3 * self.WIDTH), int(
                self.shrink_f * self.HEIGHT
            )
        ...
        if self.plot_audio_history:
            new_pos = int(self.move_fraction * self.WIDTH - (0.0133 * self.WIDTH)), int(
                self.move_fraction * self.HEIGHT
            )
        ...
        self.text = self.fps_font.render(
            "Fps: %.1f" % (self.fps),
            True,
            (255, 255, 255),
            (self.bg_color, self.bg_color, self.bg_color),
        ...
        self.textRect.x, self.textRect.y = round(0.015 * self.WIDTH), round(
            0.03 * self.HEIGHT
        )
        pygame.display.set_caption(
            "Spectrum Analyzer -- (FFT-Peak: %05d Hz)" % self.ear.strongest_frequency
    ...
    def plot_bars(self):
        ...
        for i in range(len(self.frequency_bin_energies)):
            ...
            if self.add_slow_bars:
                self.decay = min(
                    0.99, 1 - max(0, self.decay_speed * 60 / self.ear.fft_fps)
                )
